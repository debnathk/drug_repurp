...
class DBTA:
	...
	def train(self, train, val = None, test = None, verbose = True):
		...
		training_generator = data.DataLoader(utils.data_process_loader(train.index.values, train.Label.values, train, **self.config), **params)
		if val is not None:
			validation_generator = data.DataLoader(utils.data_process_loader(val.index.values, val.Label.values, val, **self.config), **params)
		
		if test is not None:
			info = utils.data_process_loader(test.index.values, test.Label.values, test, **self.config)
			...
			testing_generator = data.DataLoader(utils.data_process_loader(test.index.values, test.Label.values, test, **self.config), **params_test)
	...
	def predict(self, df_data):
		...
		'''
		print('predicting...')
		info = utils.data_process_loader(df_data.index.values, df_data.Label.values, df_data, **self.config)
